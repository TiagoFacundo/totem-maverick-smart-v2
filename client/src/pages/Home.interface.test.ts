...
describe("interface vertical de 7 polegadas", () => {
  it("mantém regras CSS de alvo mínimo e renderiza todos os controles reconstruídos com auditoria de toque", () => {
    expect(cssSource).toContain(".teal-action,.neutral-button,.danger-button { min-height: 48px;");
  ...
  it("não contém tela de confirmação e aguarda o comando start_pour no estado IDLE", () => {
    expect(homeSource).not.toContain('"confirming"');
    expect(homeSource).not.toContain("HoldConfirm");
    expect(homeSource).not.toContain("Confirmar autorização");
