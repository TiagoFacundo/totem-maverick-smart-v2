...
export function Pouring({ sessionId, poured, maxValueCents, onFinish, onEmergency }: { sessionId: string; poured: number; maxValueCents: number; onFinish: () => void; onEmergency: () => void }) {
  return <main className="ref-centered ref-pouring"><div className="eyebrow">SESSÃO ATIVA</div><h1>Servindo seu chopp</h1><p className="session-code">#{sessionId.slice(0, 8)}</p><div className="session-values"><Metric label="Volume" value={`${Math.round(poured)} ml`} /><Metric label="Total" value={formatCurrency(calculateValueCents(poured, PRODUCT.pricePer100mlCents))} /></div><div className="limit-line"><span>Limite autorizado</span><strong>{formatCurrency(maxValueCents)}</strong></div><button data-touch-target="48" className="danger-button" onClick={onFinish}>Encerrar agora</button><button data-touch-target="48" className="flow-status" onClick={onEmergency}><Radio /> Fluxo ativo</button></main>;
}
...
export default function Home() {
  ...
  const finish = useCallback(async (error = false) => {
    ...
    const payload = { session_id: sessionId, status: error ? "error" : "finished", volume_poured_ml: Math.round(poured), value_cents: calculateValueCents(poured, PRODUCT.pricePer100mlCents) };
    const key = `${sessionId}-finished`;
    try { await tapApi.finished(payload, key); setScreen(error ? "error" : "completed"); }
    catch { queuePendingFinished(payload, key); setScreen("offline"); }
  ...
  useEffect(() => {
    ...
    const pollAuthorization = () => tapApi.getCommand().then(command => {
      ...
      if (transition) { setSessionId(transition.sessionId); setMaxVolumeMl(transition.maxVolumeMl); setMaxValueCents(transition.maxValueCents); setPoured(0); setScreen(transition.screen); }
  ...
  useEffect(() => { if (screen !== "pouring" || !authorizedPourLimitMl) return; const flow = window.setInterval(() => setPoured(current => Math.min(authorizedPourLimitMl, current + 1.5)), 200); return () => window.clearInterval(flow); }, [authorizedPourLimitMl, screen]);
  useEffect(() => { if (screen === "pouring" && authorizedPourLimitMl > 0 && poured >= authorizedPourLimitMl) void finish(); }, [authorizedPourLimitMl, finish, poured, screen]);
